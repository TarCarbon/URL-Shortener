...
public class AuthService {
    ...
    public RegistrationResponse register(RegistrationRequest request) {
        UserEntity existingUser = userService.findByUsername(request.getUsername());
    ...
    private Optional<RegistrationResponse.Error> validateRegistrationFields(RegistrationRequest request) {
        if (Objects.isNull(request.getUsername())
                || request.getUsername().length() > MAX_USER_NAME_LENGTH
                || request.getUsername().length() < MIN_USER_NAME_LENGTH) {
